...
for line1 in input:
    for line2 in input:
        if line1 == line2:
            continue
        line_arr1 = literal_eval(line1)
        line_arr2 = literal_eval(line2)
        tree = Node(-1)
        tree.left = parse_array(line_arr1)
        tree.right = parse_array(line_arr2)
        ok = False
        while not ok:
            ok = True
            if tree.height > 4:
                ...
                ok = split(tree)
        max_magnitude = max(max_magnitude, magnitude(tree))
print(max_magnitude)
